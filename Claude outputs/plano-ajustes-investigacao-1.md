...
- **Mudança estrutural pedida depois de ver o resultado com mais de um item selecionado**: até aqui o desenho gerava **um diagrama de Ishikawa por item selecionado na Seção 4** (um pra "Evento 1", outro pra "PPC 1", outro pra "PPC 2"...). Ao testar com vários itens marcados de verdade, ficou claro que isso não é o que a Seção 5 deveria mostrar como "resultado final" — o esperado é **um único diagrama consolidado** pra investigação inteira, do jeito que está no mockup original (uma cabeça só representando o incidente investigado, com uma caixinha por categoria do Protocolo de Londres). Reescrevi `IshikawaDiagram.tsx`: agora ele junta os achados de **todos os itens selecionados** por categoria (ex.: se "Evento 1" e "PPC 1" marcaram "Fatores da equipe", os dois achados aparecem como marcadores dentro da mesma caixinha "Fatores da equipe", prefixados com o nome do item quando há mais de um selecionado, pra não perder a rastreabilidade de qual evento/PPC gerou qual achado). A cabeça da espinha agora mostra o texto de "Informe o incidente em investigação" (Seção 1), que é o resumo do incidente como um todo, em vez do nome de um item específico. `tsc` seguiu limpo.
